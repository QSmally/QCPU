...
It's enabled with the `--virtualise` flag and the terminal user interface is put on the terminal
altscreen. To step through the program, use the `--step` flag, otherwise the program will run
automatically until `--maxcycles 4096` total cycles or a breakpoint.

Currently, only `--mode direct` is supported. Memory virtualisation (`--mode exec`) is not.

Any emulation error, including the breakpoint instruction (`bkpt`), results in the emulator exiting
and dumping the CPU state along with the reason of exit.

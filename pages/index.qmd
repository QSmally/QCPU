...
| `11001-III II-SSS-WWW` | **xch** dw, imm(as1)    | Atomic exchange with static offset              | `dw xch *(as1 + imm)` sign ext. dw    | dw = zr is atomic clear; range imm +-16 bytes; cut8   |
| `11010-III II-SSS-WWW` | **xchw** dw, imm(as1)   | Atomic exchange word with static offset         | `dw xch *(as1 + imm)`                 | dw = zr is atomic clear; range imm +-32 bytes         |
...
| `11111-III-11-III-DDD` | **csrr** dd, uimm       | *** Control & Status Register read              | `dd = csr(uimm)`                      | CSR read dest zr are zero-operand instructions        |
| `11111-000-11-000-000` | **reti**                | *** Return from interrupt                       | `ip = csr(irp)`                       |                                                       |
| `11111-000-11-001-000` | **waiti**               | *** Wait for interrupt                          |                                       |                                                       |
| `11111-000-11-010-000` | **ftlb**                | *** Flush TLB                                   |                                       |                                                       |
| `11111-000-11-011-000` | **fence**               | *** Fence                                       |                                       | flush pending store operations                        |
| `11111-000-11-100-000` |                         |                                                 |                                       |                                                       |
| `11111-000-11-101-000` |                         |                                                 |                                       |                                                       |
| `11111-000-11-110-000` |                         |                                                 |                                       |                                                       |
| `11111-000-11-111-000` |                         |                                                 |                                       |                                                       |
| `11111-001-11-000-000` | **nop**                 | No operation                                    | literally nothing                     |                                                       |
| `11111-001-11-001-000` | **shf**                 | Set half carry flag                             | `h = 1`                               |                                                       |
| `11111-001-11-010-000` | **rhf**                 | Reset half carry flag                           | `h = 0`                               |                                                       |
| `11111-001-11-011-000` | **ssf**                 | Set sign flag                                   | `s = 1`                               |                                                       |
| `11111-001-11-100-000` | **rsf**                 | Reset sign flag                                 | `s = 0`                               |                                                       |
...
| `00000-001-11-000-001` | **ret**           | Return                                       | `add ip, zr, rp`      |       |
...
<!-- TODO: return conditionally since moving rp to DRs -->
...
special purpose registers mapped as ARs, such as the instruction pointer (write to jump) and stack
pointer (Address Register with special bound checks). ARs have reserved registers such as the frame
pointer which can be used in memory move operations. Overflows on ARs can emit hardware exceptions
to the microprocessor. In a special case, the stack pointer register can emit a hardware exception
if low or high limits are exceeded, which are configured in CSRs.

The return pointer (generated from link instructions), although an address, is a Data Register; this
design is due to the fact that the return pointer is not used as a memory address, and is likely to
be stored to memory.
...
| Data Register/Async | `r1`     | **rp** (return ptr)      | 16 bits * |
| Data Register/Async | `r2`     | **d1**                   | 16 bits   |
| Data Register/Async | `r3`     | **d2**                   | 16 bits   |
| Data Register/Async | `r4`     | **d3**                   | 16 bits   |
| Data Register/Async | `r5`     | **d4**                   | 16 bits   |
| Data Register/Async | `r6`     | **d5**                   | 16 bits   |
| Data Register/Async | `r7`     | **d6**                   | 16 bits   |
| Address Register    | `r8`     | **zr** (zero)            | 16 bits   |
| Address Register    | `r9`     | **ip** (instruction ptr) | 16 bits * |
| Address Register    | `r10`    | **sp** (stack ptr)       | 16 bits   |
| Address Register    | `r11`    | **fp** (frame ptr)       | 16 bits   |
| Address Register    | `r12`    | **a1**                   | 16 bits   |
| Address Register    | `r13`    | **a2**                   | 16 bits   |
| Address Register    | `r14`    | **a3**                   | 16 bits   |
| Address Register    | `r15`    | **a4**                   | 16 bits   |
...
registers are translated to `add` instructions with a zero operand, which in a special case does not
update flags. The `test` pseudoinstruction is therefore executed with `sub`.

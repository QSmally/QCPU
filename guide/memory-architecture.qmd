...
```
                  lui t1, 0xEA            ; load upper byte
                  mld x1, t1, 0           ; loads 0xEA00 into x1
```

```
                  lui t1, 0xBE            ; load upper byte
...
Prefetching from memory is done by loading into `zr`, which discards the result.
...
Atomic instructions (`aswp` family of instructions) are also async, but atomic

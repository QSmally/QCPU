...
rather than letting the kernel handle it. Though this makes it very, very easy
to hack around so it should be used with care.

A kernel returns the device id and segment address of physical memory. Having
different allocators in userspace can increase performance by minimising system
calls.
...
::: {.note}
Note: figure out permission checks (flags, physical) without storing them in
userspace
:::

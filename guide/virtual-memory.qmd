...
- `exec(device id)`: set 'kernel' section to the boot sector of a specific
  device id.
- `setk(segment start address)`: set 'kernel' section to 12 KiB of linear
  physical memory, from a starting segment in the current device (likely taken
  from the memory map).
...
- Instance data `[1]`
    - globals, stack base, frame, insertion ptrs
    - process, thread id (userland)
    - context register copy (userland, sysc)
- Globals (mutable) `[1]`
...
::: {.note}
Shared objects and address offsets.
:::

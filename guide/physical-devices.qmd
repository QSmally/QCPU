...
|  64 KiB  |             |  16 MiB  | ___ I/O 1 'dmac' regfile |
|          |             |          | ___ I/O ... regfile      |
| virtual  | ___ MMU ___ | physical | ___ I/O 23 regfile       | physical
...
@arm-memory-management [chap. 5]. In this case, there's a 16 bit physical
...
Currently, the two-byte MMU stride is for a 16 bit physical address[^3].

[^3]: Physical/resolved addresses will be converted to 16 bits rather than 24.
...
interrupt. The I/O device address range is from `0x000000` to `0x001700` (6.144
bytes).
...
|-----------+---------+------+------------------------------------------------------------------------------------|
| `0100000` | `reset` | `1`  | reset pin active (highest priority)                                                |
| `0100001` | `lcktl` | `1`  | interrupt 'locked too long' exception                                              |
| `0100010` |         |      | reserved                                                                           |
| `0100011` |         |      | reserved                                                                           |
| `0100100` | `segv`  | `0`  | on unmapped or unprivileged virtual memory access                                  |
| `0100101` | `cow`   | `1`  | a memory page is being written to with copy-on-write active                        |
| `0100110` | `mmap`  | `0`  | a (physical) map request of an I/O device was received                             |
| `0100111` | `systm` | `0`  | system timer for preemptive multitasking (lowest system priority, higher than I/O) |
...
include multiple physical I/O devices to save register file space.

Interrupt locks can delay incoming interrupt requests **from 0b0001000 onwards**
(that is, important system interrupts can still be executed in the I/O 0 and 1
devices, unless the `mask` is 0). There's an internal clock timer running when
the lock is active, and if the lock is high for too long[^2], a `lcktl` ('locked
too long') interrupt may get thrown.

[^1]: Feature is currently uncertain.
[^2]: Timeout of `lcktl` is system dependent.
